...
RelativeDistanceWidget::RelativeDistanceWidget(QWidget *parent) : QWidget(parent){
    ...
    label = new QLabel;
    label->setText("Координаты корабля: ");
    table = new QTableWidget;
    table->setRowCount(1);
    table->setColumnCount(3);
    table->setHorizontalHeaderLabels({"ID Объекта", "Относительное расстояние", "Координаты"});
    lay->addWidget(label);
    lay->addWidget(table);
    this->setLayout(lay);
}
...
void RelativeDistanceWidget::getShipCoords(const std::vector<TrajData> &trajectories) {
    table->setRowCount(trajectories.size());
    for (const auto& i : trajectories) {
        if (i.type == SHIP) {
            ship_id = i.numTr;
            table->hideRow(ship_id);
    ...
    y_ship = geoc_ship.y_;
    z_ship = geoc_ship.z_;
    label->setText("Координаты корабля: " + QString::number(geod_ship.latitude) + ", " + QString::number(geod_ship.longitude));
}

void RelativeDistanceWidget::getOtherObjsCoords(int id, double latitude, double longitude, double altitude) {
    if (id != ship_id) {
        table->setItem(id, 0, new QTableWidgetItem(QString::number(id)));
        double dist = calculateRDistance(latitude, longitude, altitude);
        table->setItem(id, 1, new QTableWidgetItem(QString::number(dist)));
        table->setItem(id, 2, new QTableWidgetItem(QString::number(latitude) + " " + QString::number(longitude) + " " + QString::number(altitude)));
    }
}
